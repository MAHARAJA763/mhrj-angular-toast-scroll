...
export class ToastScrollService
{
  ...
  raise( message?: string, data?: ToastConfig )
  {
    ...
    const scrollFrom = this.data.scrollFrom ? this.data.scrollFrom : ( this.data.verticalPosition === 'top' ) ? 'top' : 'bottom';
    const scrollTo = this.data.scrollTo ? this.data.scrollTo : scrollFrom;
    const entryClass = `toast-entry-${verticalPosition}-${horizontalPosition}-${scrollFrom}`;
    const exitClass = `toast-exit-${verticalPosition}-${horizontalPosition}-${scrollTo}`;
